...
class PresenceService {
   ...
   */
  async updateUserPresence(status: OnlineStatus, customMessage?: string): Promise<void> {
    ...
    try {
      ...
      this.pendingUpdates.set(updateKey, setTimeout(async () => {
        ...
        if (error) {
          secureLogger.error('Failed to update user presence', { error, status });
        } else {
  ...
  async getUsersPresence(userIds: string[]): Promise<UserPresence[]> {
    ...
    try {
      ...
      if (uncachedIds.length > 0) {
        ...
        if (error) {
          secureLogger.error('Failed to fetch user presence', { error, userIds: uncachedIds });
        } else {
          fetched = data || [];
  ...
  async subscribeToChatPresence(
    ...
    try {
      ...
      secureLogger.error('Error subscribing to chat presence', { error, chatId });
      throw error;
    }
  }
  ...
  private setupVisibilityHandling(): void {
    if (typeof document !== 'undefined') {
      document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') {
          ...
          this.lastActivity = Date.now() - PRESENCE_CONFIG.AWAY_THRESHOLD_MS + 5000; // Grace period
        }
      });
    }
    ...
    if (typeof window !== 'undefined') {
      ...
      window.addEventListener('blur', () => {
        this.lastActivity = Date.now() - PRESENCE_CONFIG.AWAY_THRESHOLD_MS + 5000;
      });
    }
  }
  ...
  private async checkAndUpdateStatus(): Promise<void> {
    ...
    if (timeSinceActivity > PRESENCE_CONFIG.OFFLINE_THRESHOLD_MS) {
      if (this.currentStatus !== OnlineStatus.OFFLINE && this.currentStatus !== OnlineStatus.INVISIBLE) {
      ...
      if (this.currentStatus === OnlineStatus.AWAY || this.currentStatus === OnlineStatus.OFFLINE) {
  ...
  private async getChatParticipantIds(chatId: string): Promise<string> {
    try {
      ...
      if (error) {
        secureLogger.error('Failed to get chat participant IDs', { error, chatId });
        return '';
      }

      return (data || []).join(',');
    } catch (error) {
      secureLogger.error('Error getting chat participant IDs', { error, chatId });
      return '';
    }
  }
  ...
  async cleanup(): Promise<void> {
    ...
    if (user) {
      try {
        ...
        secureLogger.error('Failed to set offline status during cleanup', { error });
      }
    }
