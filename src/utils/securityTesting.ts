...
class SecurityTestFramework {
   ...
   */
  async runSecurityTests(config: Partial<TestConfig> = {}): Promise<SecurityTestReport> {
    ...
    try {
      ...
      const suites = await Promise.all([
        ...
        ...(testConfig.includePerformanceTests ? [this.runPerformanceTests()] : []),
        ...(testConfig.includeStressTests ? [this.runStressTests()] : [])
      ]);

      // Generate report
      const report = this.generateSecurityReport(suites, Date.now() - startTime);
  ...
  private async runEncryptionTests(): Promise<TestSuiteResult> {
    ...
    tests.push(await this.runTest('encryption-basic', 'encryption', async () => {
      ...
      return { encrypted: encrypted.algorithm, keyDerivation: encrypted.keyDerivation };
    }));

    // Test 2: Maternal health ID encryption
    tests.push(await this.runTest('encryption-maternal-id', 'encryption', async () => {
      const maternalId = '1234567890';
      const password = 'user-password-123';
      
      const encrypted = await encryptionService.encryptMaternalHealthId(maternalId, password);
      const decrypted = await encryptionService.decryptMaternalHealthId(encrypted, password);
    ...
    tests.push(await this.runTest('encryption-session-token', 'encryption', async () => {
      const sessionToken = 'session-token-xyz-789';
      const userId = 'user-123-456';
      
      const encrypted = await encryptionService.encryptSessionToken(sessionToken, userId);
      const decrypted = await encryptionService.decryptSessionToken(encrypted, userId);
    ...
    tests.push(await this.runTest('encryption-wrong-password', 'encryption', async () => {
    ...
    tests.push(await this.runTest('encryption-hash-verification', 'encryption', async () => {
      ...
      const isInvalid = await encryptionService.verifyHash('wrong-data', hash1);
      if (isInvalid) {
  ...
  private async runAuthenticationTests(): Promise<TestSuiteResult> {
    ...
    tests.push(await this.runTest('auth-initialization', 'authentication', async () => {
    ...
    tests.push(await this.runTest('auth-username-validation', 'authentication', async () => {
      ...
      const invalidResult = authService.validateUsernameClient(invalidUsername);
    ...
    tests.push(await this.runTest('auth-maternal-id-validation', 'authentication', async () => {
      const validId = '1234567890';
      const invalidId = '123456789'; // Too short
      const invalidChars = '123456789a'; // Contains letters
      
      const validResult = authService.validateMaternalHealthIdClient(validId);
      const invalidResult1 = authService.validateMaternalHealthIdClient(invalidId);
      const invalidResult2 = authService.validateMaternalHealthIdClient(invalidChars);
    ...
    tests.push(await this.runTest('auth-password-validation', 'authentication', async () => {
      const strongPassword = 'StrongPass123!';
      const weakPassword = 'weak';
      
      const strongResult = authService.validatePasswordClient(strongPassword);
    ...
    tests.push(await this.runTest('auth-registration-request', 'authentication', async () => {
      ...
      const privacyCheck = validatePrivacyCompliance(sanitizeObject(request));
  ...
  private async runValidationTests(): Promise<TestSuiteResult> {
    ...
    tests.push(await this.runTest('validation-service-stats', 'validation', async () => {
      const stats = validationService.getStats();
      return stats;
    }));

    // Test 2: Input sanitization
    tests.push(await this.runTest('validation-input-sanitization', 'validation', async () => {
    ...
    tests.push(await this.runTest('validation-constraints', 'validation', async () => {
      ...
      const shortResult = validationService.validateUsernameClient(shortUsername);
      const longResult = validationService.validateUsernameClient(longUsername);
    ...
    tests.push(await this.runTest('validation-privacy-compliance', 'validation', async () => {
      ...
      if (compliance.isCompliant) {
        throw new Error('Privacy compliance check should have detected violations');
      }

      return { 
        violations: compliance.violations.length,
        hasMaternalIdViolation: compliance.violations.some(v => v.includes('maternal'))
      };
    }));
  ...
  private async runSessionManagementTests(): Promise<TestSuiteResult> {
    ...
    tests.push(await this.runTest('session-config', 'session', async () => {
    ...
    tests.push(await this.runTest('session-no-active-session', 'session', async () => {
    ...
    tests.push(await this.runTest('session-needs-refresh', 'session', async () => {
  ...
  private async runPrivacyProtectionTests(): Promise<TestSuiteResult> {
    ...
    tests.push(await this.runTest('privacy-maternal-id-masking', 'privacy', async () => {
    ...
    tests.push(await this.runTest('privacy-data-sanitization', 'privacy', async () => {
      ...
      if (sanitized.maternal_health_id !== '[REDACTED]' ||
      ...
      if (sanitized.username !== 'testuser' || sanitized.normal_field !== 'normal data') {
    ...
    tests.push(await this.runTest('privacy-secure-logging', 'privacy', async () => {
  ...
  private async runServiceIntegrationTests(): Promise<TestSuiteResult> {
    ...
    tests.push(await this.runTest('integration-supabase-health', 'integration', async () => {
    ...
    tests.push(await this.runTest('integration-service-status', 'integration', async () => {
      const status = authService.getInitializationStatus();
    ...
    tests.push(await this.runTest('integration-service-stats', 'integration', async () => {
  ...
  private async runPerformanceTests(): Promise<TestSuiteResult> {
    ...
    tests.push(await this.runTest('performance-encryption', 'performance', async () => {
      const iterations = 10;
      const startTime = Date.now();
    ...
    tests.push(await this.runTest('performance-validation', 'performance', async () => {
      const testCases = [
        'validuser123',
        'invalid@user',
  ...
  private async runStressTests(): Promise<TestSuiteResult> {
    ...
    tests.push(await this.runTest('stress-concurrent-encryption', 'stress', async () => {
      const concurrentOps = 20;
      const promises = [];
      
      for (let i = 0; i < concurrentOps; i++) {
        promises.push(
          encryptionService.encrypt(`test-data-${i}`, `password-${i}`)
            .then(encrypted => encryptionService.decrypt(encrypted, `password-${i}`))
        );
      }
  ...
  private generateSecurityReport(
    ...
    const criticalFailures = allTests.filter(t => 
      t.status === 'fail' && criticalCategories.includes(t.category)
    );
  ...
  private calculateSecurityScore(
    ...
    const criticalWarnings = suites
      .flatMap(s => s.tests)
      .filter(t => t.status === 'warning' && ['encryption', 'authentication', 'privacy'].includes(t.category));
 ...
 */
export const runSecurityTests = async (config?: Partial<TestConfig>): Promise<SecurityTestReport> => {
...
export const quickSecurityCheck = async (): Promise<{
  ...
  const status = report.securityScore >= 80 ? 'secure' : 
                 report.securityScore >= 60 ? 'warning' : 'critical';
