import { View, Text, FlatList, Pressable, Animated, TextInput } from 'react-native';
...
export default function AnonFeedScreen({ onComment, onOpenPost }: { onComment?: () => void; onOpenPost?: () => void }) {
  const theme = useTheme() as any;
  const { colors } = theme;
  const fade = new Animated.Value(0);
  Animated.timing(fade, { toValue: 1, duration: 200, useNativeDriver: true }).start();
  return (
    <Animated.View style={{ flex: 1, backgroundColor: 'transparent', paddingTop: 40, paddingBottom: 72, opacity: fade }}>
      <FlatList
        data={items}
        keyExtractor={(i) => i.id}
        ...
        renderItem={({ item }) => (
          <Pressable onPress={() => onOpenPost ? onOpenPost() : onComment && onComment()} style={({ pressed }) => [{ borderRadius: theme.radius.lg, overflow: 'hidden', transform: [{ scale: pressed ? 0.98 : 1 }], ...theme.shadow.card }]}> 
            <BlurView intensity={30} tint="dark" style={{ padding: theme.spacing(1.75), backgroundColor: '#ffffff10' }}>
              <Text style={{ color: colors.text, fontSize: 16, marginBottom: 8 }}>{item.body}</Text>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ color: colors.subtext, fontSize: 12 }}>消滅まで {item.time}</Text>
      ...
      />
      <View style={{ position: 'absolute', left: 0, right: 0, bottom: 56, padding: theme.spacing(1.5), backgroundColor: colors.card, borderTopColor: '#22252B', borderTopWidth: 1, opacity: 0.98 }}>
...
function Pill({ children }: { children: any }) {
  ...
  const { colors } = theme;
  return (
    <View style={{ backgroundColor: colors.surface, borderRadius: 999, paddingHorizontal: theme.spacing(1.25), paddingVertical: 6 }}>
