...
export default function LikedPostsListScreen({ commentDeltas, onOpen }: { commentDeltas?: Record<string, number>; onOpen?: (postId: string) => void }) {
  ...
  return (
    <Animated.View style={{ flex: 1, backgroundColor: 'transparent', paddingTop: 40, opacity: fade }}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable accessibilityRole="button" accessibilityLabel="ポストを開く" onPress={() => onOpen && onOpen(item.id)} style={({ pressed }) => [{ borderRadius: theme.radius.lg, overflow: 'hidden', transform: [{ scale: pressed ? 0.98 : 1 }], ...theme.shadow.card }]}> 
            <PostCard post={item} commentDelta={commentDeltas?.[item.id] || 0} onOpenComments={(id) => onOpen && onOpen(id)} onToggleLike={handleToggleLike} />
          </Pressable>
        )}
        ...
        ListEmptyComponent={!loading ? () => (
          <View style={{ alignItems: 'center', paddingTop: 80 }}>
            <Text style={{ color: colors.subtext }}>共感したポストはまだありません</Text>
          </View>
        ) : null}
      />
    </Animated.View>
