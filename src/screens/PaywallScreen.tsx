...
export default function PaywallScreen({ onClose }: { onClose?: () => void }) {
  ...
  const priceDisplay = useMemo(() => {
    if (!plan || !plan.price_cents || plan.price_cents <= 0) return '—';
    try {
  ...
  const handlePurchase = async () => {
    if (!plan) return;
    if (!eligible) {
      Alert.alert('申込要件を満たしていません', '母子手帳認証バッジが必要です。設定またはプロフィールから認証を完了してください。');
      return;
    }
    setLoading(true);
    try {
      const res = await purchase(plan.product_id || 'premium_monthly');
      if (!res.ok) Alert.alert('購入エラー', res.error || '不明なエラー');
      else Alert.alert('完了', '購入処理を受け付けました');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      if (!res.ok) Alert.alert('復元エラー', res.error || '不明なエラー');
      else Alert.alert('完了', '購入の復元を実行しました');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ScrollView
      contentContainerStyle={{ padding: theme.spacing(2), paddingTop: (insets.top || 0) + theme.spacing(1), paddingBottom: theme.spacing(8) }}
    ...
    >
      <View style={{ gap: theme.spacing(2) }}>
        ...
        >
          <Text style={{ color: colors.text, fontWeight: '700' }}>購入を復元</Text>
        </Pressable>
        <Pressable
          accessibilityRole="button"
          onPress={onClose}
