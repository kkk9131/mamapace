...
export default function CommentsListScreen({ postId, refreshKey, onCompose }: { postId: string; refreshKey?: number; onCompose?: () => void }) {
  ...
  const load = async (opts?: { refresh?: boolean }) => {
    ...
    setLoading(true);
    try {
      const res = await fetchComments(postId, { before: opts?.refresh ? null : cursor });
  ...
  useEffect(() => { load({ refresh: true }); }, [postId, refreshKey]);
  ...
  return (
    <Animated.View style={{ flex: 1, backgroundColor: 'transparent', paddingTop: 40, opacity: fade }}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View accessible accessibilityRole="summary" accessibilityLabel={`${item.user?.display_name || item.user?.username || '匿名'}のコメント`} style={{ borderRadius: theme.radius.lg, overflow: 'hidden', ...theme.shadow.card }}>
            <BlurView intensity={30} tint="dark" style={{ padding: theme.spacing(1.25), backgroundColor: '#ffffff10' }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
                <Text style={{ color: colors.subtext, fontSize: 12 }}>
                  {item.user?.display_name || item.user?.username || '匿名'} ・ {new Date(item.created_at).toLocaleString()}
                </Text>
                {item.user_id === user?.id && (
                  ...
                  }} style={({ pressed }) => [{ paddingHorizontal: 8, paddingVertical: 4, borderRadius: 999, backgroundColor: colors.surface, transform: [{ scale: pressed ? 0.97 : 1 }] }]}> 
        ...
        ListEmptyComponent={!loading ? () => (
          <View style={{ alignItems: 'center', paddingTop: 80 }}>
            <Text style={{ color: colors.subtext }}>まだコメントがありません</Text>
          </View>
        ) : () => (
            ...
            {[1,2,3,4].map(i => (
              <View key={i} style={{ marginBottom: theme.spacing(1) }}>
                <View style={{ borderRadius: theme.radius.lg, overflow: 'hidden', ...theme.shadow.card }}>
                  <BlurView intensity={30} tint="dark" style={{ padding: theme.spacing(1.25), backgroundColor: '#ffffff10' }}>
                    <SkeletonLine width={120} height={10} style={{ marginBottom: 6 }} />
      ...
      />
      {onCompose && (
        <Pressable accessibilityRole="button" accessibilityLabel="コメントを作成" onPress={onCompose} style={({ pressed }) => [{ position: 'absolute', right: 20, bottom: 88, backgroundColor: colors.pink, borderRadius: 28, width: 56, height: 56, alignItems: 'center', justifyContent: 'center', transform: [{ scale: pressed ? 0.97 : 1 }], ...theme.shadow.card }]}> 
