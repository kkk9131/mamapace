...
export default function ChatsListScreen({ onOpen, filters = {} }: ChatsListScreenProps) {
  ...
  useEffect(() => {
    const loadLastViewedTimes = async () => {
      try {
        const stored = await AsyncStorage.getItem(`last_viewed_times_${user?.id}`);
        if (stored) {
  ...
  const hasNewMessage = useCallback((chat: ChatWithParticipants) => {
    // 最終メッセージがない場合は新規なし
  ...
  // Handle chat press
  const handleChatPress = useCallback(async (chat: ChatWithParticipants) => {
    // 現在の時刻を記録（NEW表示を消すため）
    ...
    if (onOpen) {
      ...
      const userName = otherParticipant?.display_name || otherParticipant?.username || 'ユーザー';
  ...
  const renderChatItem = ({ item }: { item: ChatWithParticipants }) => {
    try {
      ...
      const displayName = otherParticipant?.display_name || otherParticipant?.username || 'ユーザー';
    ...
    return (
      <Pressable 
        ...
        style={({ pressed }) => ({
          padding: 16,
          backgroundColor: pressed ? colors.surface : (isNew ? '#ffffff05' : 'transparent'),
          borderBottomWidth: 1,
      ...
      return (
        <View style={{ padding: 16, backgroundColor: colors.surface }}>
          <Text style={{ color: colors.text }}>エラー: チャットアイテムの表示に失敗</Text>
        </View>
      );
    }
  };
  ...
  const renderEmpty = useCallback(() => {
    ...
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', paddingTop: 100 }}>
        ...
        <Text style={{ color: colors.subtext, textAlign: 'center', paddingHorizontal: 32 }}>
          新しいチャットを始めてみましょう
        </Text>
        <Pressable
